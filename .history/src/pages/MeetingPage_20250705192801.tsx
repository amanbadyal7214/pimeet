        ...
        <div className="flex flex-col flex-1 rounded-lg bg-gray-100 p-4">
          <div className="relative flex-1 rounded-lg overflow-hidden bg-black">
            {spotlightUserId === 'local' ? (
              ...
              spotlightUserId && remoteStreams.has(spotlightUserId) && (
                <video
                  ref={(el) => {
                    if (el) {
            ...
            <div className="absolute top-4 right-4 flex space-x-4 z-20">
              <button className="bg-black bg-opacity-50 rounded-full p-2 hover:bg-opacity-75">
                {/* Fullscreen icon */}
              ...
              <button className="bg-black bg-opacity-50 rounded-full p-2 hover:bg-opacity-75">
                {/* Headphones icon */}
                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 13a9 9 0 0118 0v5a2 2 0 01-2 2h-1a2 2 0 01-2-2v-3a2 2 0 00-2-2H9a2 2 0 00-2 2v3a2 2 0 01-2 2H4a2 2 0 01-2-2v-5z" />
                </svg>
              </button>
              <button className="bg-black bg-opacity-50 rounded-full p-2 hover:bg-opacity-75">
                {/* Share icon */}
                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 12v.01M12 12v.01M20 12v.01M12 20v.01M12 4v.01M16.24 7.76l.01.01M7.76 16.24l.01.01M16.24 16.24l.01.01M7.76 7.76l.01.01" />
                </svg>
              </button>
              <button className="bg-red-600 rounded-full p-2 hover:bg-red-700">
                {/* End call icon */}
                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<attempt_completion>
<result>
I have redesigned the MeetingPage UI to closely match the provided photo, including:
- A left panel with a grid of participant videos and a daily task list below.
- A right panel with a large spotlight video for the selected participant (host or others).
- Control buttons overlay on the spotlight video.
- A message panel below the spotlight video.
- Responsive layout and styling consistent with the photo.

Please test the updated UI and confirm if it meets your expectations.
</result>
