...
console.log('MeetingPage rendered with meetingId:', spotlightUserId);
  ...
  useEffect(() => {
    if (localStream && localVideoRef.current) {
      ...
      console.warn('Local stream or localVideoRef.current is null');
    }
  }, [localStream]);
  ...
  useEffect(() => {
    remoteStreams.forEach((stream, userId) => {
      ...
      if (videoElement) {
        ...
        // Check if video tracks are present and enabled
        const videoTracks = stream.getVideoTracks();
        if (videoTracks.length === 0) {
          console.warn(`User video not seen for userId: ${userId} - No video tracks`);
        } else if (!videoTracks.some(track => track.enabled)) {
          console.warn(`User video not seen for userId: ${userId} - Video tracks disabled`);
        }
      } else {
  ...
  const renderParticipantVideo = (stream: MediaStream, userId: string, displayName: string, role: string, isLocal: boolean = false) => {
    ...
    if (!videoEnabledForUser) {
      console.warn(`Rendering hidden video for userId: ${userId} (${displayName}) because video is disabled or no video tracks`);
    }
      ...
      <div className="flex flex-1 gap-4">
        <div className="flex flex-col w-1/3 gap-4 overflow-auto">
          <div className="grid grid-cols-2 gap-4">
            ...
            {/* Remove duplicate local video rendering below to avoid confusion */}
            ...
            {Array.from(remoteUserDisplayNames).map(([userId, name]) => {
              ...
              return (
                <div key={userId}>
                  {renderParticipantVideo(stream, userId, name, 'Participant', false)}
                </div>
              );
            })}
          </div>
            ...
            {spotlightUserId === 'local' ? (
              ...
              spotlightUserId && remoteStreams.has(spotlightUserId) && (
                <>
                  ...
                  />
                  <div className="absolute bottom-4 left-4 z-20 text-white text-sm bg-black bg-opacity-50 px-3 py-1 rounded-full">
                    {remoteUserDisplayNames.get(spotlightUserId) || 'Unknown'}
                    {spotlightUserId === hostUserId && ' (Host)'}
                  </div>
                </>
              )
